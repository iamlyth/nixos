# Pure nixvim configuration. Consumed two ways:
#   1. home-manager/repo/nvim.nix wraps this into `programs.nixvim`.
#   2. flake.nix exposes `packages.${system}.nvim` via `makeNixvim`.
# Keep this attrset free of `pkgs`, `lib`, and `config` references so both
# consumers can `import` it with no arguments.
{
